...
#include <LibQTelegram/autogenerated/types/user.h>
//#include <LibQTelegram/objects/telegraminitializer.h>
#include <LibQTelegram/objects/telegraminitializer.h>
#include <LibQTelegram/cache/telegramcache.h>
#include <LibQTelegram/models/contactsmodel.h>
#include <LibQTelegram/telegram.h>
...
void MorseConnection::doConnect(Tp::DBusError *error)
{
    ...
    TelegramInitializer *in = new TelegramInitializer(this);
    TelegramInitializer &appInfo = *in;
    appInfo.setApiId(14617);
    appInfo.setApiHash(QLatin1String("e17ac360fd072f83d5d08db45ce9a121"));
//    appInfo.setApiVersion(QLatin1String("0.1"));
//    appInfo.setDeviceInfo(QLatin1String("pc"));
//    appInfo.setOsInfo(QLatin1String("GNU/Linux"));
//    appInfo.setLanguageCode(QLatin1String("en"));

    appInfo.setPhoneNumber(m_selfPhone);
...
//#endif
    ...
    m_contacts->setTelegram(m_core);

    setStatus(Tp::ConnectionStatusConnecting, Tp::ConnectionStatusReasonNoneSpecified);

    connect(in, SIGNAL(signUpRequested()), this, SLOT(whenPhoneCodeRequired()));
    connect(in, SIGNAL(sessionPasswordNeeded(QString)), this, SLOT(onPasswordInfoReceived(QString)));
...
void MorseConnection::onSelfUserAvailable()
{
    ...
    MorseIdentifier selfIdentifier = MorseIdentifier::fromUserId(m_core->me()->id());
...
void MorseConnection::onPasswordInfoReceived(QString hint)
...
//    m_core->signIn(m_selfPhone, QString::fromLatin1(data.constData()));
    m_core->initializer()->signIn(QString::fromLatin1(data.constData()));
}
...
void MorseConnection::startMechanismWithData_password(const QString &mechanism, const QByteArray &data, Tp::DBusError *error)
{
    ...
    m_core->initializer()->sendPassword(data);
...
//    m_core->setOnlineStatus(m_wantedPresence == c_onlineSimpleStatusKey);
    m_core->setOnline(m_wantedPresence == c_onlineSimpleStatusKey);
...
{
    User *userInfo = TelegramCache::cache()->user(userId);
...
//    m_core->getUserInfo(&info, identifier.userId());
    User *info = TelegramCache::cache()->user(identifier.userId());
...
//        m_core->setOnlineStatus(status == c_onlineSimpleStatusKey);
        m_core->setOnline(status == c_onlineSimpleStatusKey);
    }
...
//    const QVector<quint32> ids = m_core->contactList();
    ...
    for (int i = 0; i < m_contacts->rowCount(); ++i) {
        User *user = m_contacts->index(i).data(ContactsModel::ItemRole).value<User*>();
        if (!user) {
            qDebug() << "Unable to get user";
        }
        ids << user->id();
    }
...
//    m_core->setOnlineStatus(false); // TODO: Real disconnect
    m_core->setOnline(false); // TODO: Real disconnect
...
//    return m_core && (m_core->connectionState() == TelegramNamespace::ConnectionStateReady);
    return m_core && m_core->loggedIn() && !m_core->syncing();
}
